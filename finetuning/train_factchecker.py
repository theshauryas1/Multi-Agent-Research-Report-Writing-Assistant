"""
Train Fact-Checker Agent
Fine-tunes Phi-2 for hallucination detection using FEVER dataset
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(PATH_CONFIG.logs_dir, 'train_factchecker.log')),
...
def train_factchecker_agent():
    """
    Main training function for Fact-Checker Agent
    """
    logger.info("=" * 60)
    logger.info("Training Fact-Checker Agent")
    logger.info("=" * 60)
    
    # Create output directory
    os.makedirs(PATH_CONFIG.factchecker_adapter_path, exist_ok=True)
    ...
    training_args = TrainingArguments(
        output_dir=PATH_CONFIG.factchecker_adapter_path,
    ...
    # Save model
    logger.info("\n💾 Saving model...")
    trainer.save_model(PATH_CONFIG.factchecker_adapter_path)
    
    # Save training metrics
    metrics = train_result.metrics
    metrics_path = os.path.join(PATH_CONFIG.factchecker_adapter_path, 'training_metrics.json')
    with open(metrics_path, 'w') as f:
        json.dump(metrics, f, indent=2)
    
    logger.info("\n" + "=" * 60)
    logger.info("✅ Training complete!")
    logger.info(f"Model saved to: {PATH_CONFIG.factchecker_adapter_path}")
    logger.info("=" * 60)
...
if __name__ == "__main__":
    try:
        success = train_factchecker_agent()
